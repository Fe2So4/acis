...
export default {
  ...
  data () {
    return {
      ...
      list: [],
      groupNo: null,
      infusionList: [{ menuName: '生理盐水', value: '1' },
        { menuName: '氰化钾', value: '2' },
        { menuName: '百草枯', value: '3' }],
      bloodTransfusionList: [{ menuName: '血红蛋白', value: '1' },
  ...
  methods: {
    ...
    setGrid () {
      ...
      for (let i = 0; i < this.configuration.outPut.num; i++) {
        const group = new Group({
          pos: [0,
            yScale * (this.configuration.infusion.num + this.configuration.bloodTransfusion.num) + Math.round(i * yScale)],
          size: [grid.attr('width'), yScale],
    ...
    handleGridRightClick () {
      ...
      grid.addEventListener('mousedown', evt => {
        if (evt.originalEvent.button === 2) {
          ...
          } else if (evt.target.attr('className').indexOf('bloodTransfusion') !== -1) {
    ...
    handleSubmit (param) {
      ...
      if (!this.editMode) {
        ...
        if (list[this.groupNo].data) {
          ...
          const data = []
          data.push(param)
          this.list[this.groupNo].data = data
          this.list[this.groupNo].total = param.dose
        }
        this.drugDetailVisible = false
        this.setDrug()
        // this.setDrugLine()

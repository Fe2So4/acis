...
export default {
  ...
  methods: {
    ...
    getDictionaryData (start) {
      return this.getAnesMethodDict(this.content, start).then(
        ({ list, pageNum, isFirstPage, isLastPage }) => {
          list = list.map(({ anesCode, anesName }) => `${anesCode}_${anesName}`)
    ...
    getAnesMethodDict (content, start) {
      return request({
        method: 'get',
        url: getAnesMethodDict,
        params: {
          content,
          start,
          size: 10

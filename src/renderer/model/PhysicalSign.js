import { Polyline, Label, Group, Sprite, SpriteSvg, Path } from 'spritejs'
import moment from 'moment'
// const SBPUrl = require('@/iconfont/AMISSVG/SBP.svg')
...
export class PhysicalSignLine {
  ...
  addPoint({ time, value }) {
    ...
    const position = [x, y]
    const width = hasLabel ? 14 : 7
    const height = hasLabel ? 16 : 8
    let newy
    let fontSize = 14
    if (text === '∨') {
      fontSize = 14
      newy = position[1] - height / 2 - 2
    } else if (text === '∧') {
      fontSize = 14
  ...
  addSprite({ time, value }) {
    ...
    //   fill: this._color
    // })
    // const label = new SpriteSvg(SBPUrl)
    const label = new Path()

    const position = [x, y]
    const width = hasLabel ? 8 : 4
    const height = hasLabel ? 8 : 4
    label.attr({
      d: 'M6.991,13.571L7.786,14,14.009,0.429,13.214,0L7.1,14-0.01.429,0.9,0Z',
      fillColor: this._color,
      // bgcolor: this._color,
      anchor: [0, 0],
      scale: 0.5,
